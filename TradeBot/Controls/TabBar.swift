...
struct TDtabBar: View {
    ...
    var body: some View {
        ...
        ZStack(alignment: Alignment(horizontal: .center, vertical: .bottom), content: {
            PageView(presented: $menu1, backgroundColor: Color.orange, page:  AnyView(ChartView(xStepValue: 1.0, yStepValue: 0.04).standard()), Action: {
                                              })
            PageView(presented: $run, backgroundColor: Color.black, page:  AnyView(OnboardingWorkflow()
                                                                                    .standard()
                                                                                    .animation(runOnce()).transition(.slide)
            ), Action: {
            })
            ...
            ZStack {
                VStack{
                    Page.init(isPresented: self.$menu2,backgroundColor: Color.yellow, view:
                                AnyView(ListSample()
                                            .standard()
                                            .offset(y:-200)
                                ))
            ...
            ZStack {
                VStack{
                    Page.init(isPresented: self.$menu3,backgroundColor: Color.blue, view:
                                AnyView(PhoneScreen().standard().offset(x:-20)))
                }
            }.opacity(self.menu3 ? 1.0: 0.0)
            
            ZStack {
                VStack{
                    Page.init(isPresented: self.$menu4,backgroundColor: Color.green, view:
                                AnyView(ListSample().standard().offset(y:-200).hueRotation(Angle(degrees: 90)))).frame(width: UIScreen.screenWidth, height: UIScreen.screenHeight, alignment: .bottom)
